...
async function exportGroups() {
  ...
  const backup = {
    ...
    groups: groups.map((g) => ({
      ...
      name: g.name,
      appType: g.appType,
      // 归入该分组的应用（按 groupId 匹配）
      apps: apps.filter((a) => a.groupId === g.groupId).map((a) => ({ appId: a.appId, appName: a.appName })),
    })),
    // 未分组应用（groupId 为空/缺失），用于参考但默认不同步
    ungroupedApps: apps.filter((a) => !a.groupId).map((a) => ({ appId: a.appId, appName: a.appName })),
  };
  ensureDir(config.BACKUP_DIR);
  const ts = localTs();
  const accTag = account ? String(account.userName || account.displayName || 'unknown').replace(/[^\w@.-]/g, '_') : 'unknown';
  ...
  const filePath = path.join(config.BACKUP_DIR, fileName);
  fs.writeFileSync(filePath, JSON.stringify(backup, null, 2), 'utf-8');
  return { ok: true, file: fileName, count: groups.length, account: backup.meta.sourceAccount };
}
